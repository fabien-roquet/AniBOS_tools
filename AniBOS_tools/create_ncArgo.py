...
def create_ncArgo(fname,mode='w',Nprof=5, Nlevels=20, isfluo=0, isoxy=0, islight=0, issalcor=0):
    """
    Create a netCDF file for AniBOS data
    ...
    fname : str
        Name of the file
    mode : str
        Mode to open the file. Default is 'w' (write)
    Nprof : int
        Number of profiles
    Nlevels : int
        Number of levels
    isfluo : int
        1 if fluorescence data are present
    isoxy : int
        1 if oxygen data are present
    islight : int
        1 if light data are present
    issalcor : int
        1 if salinity correction is applied
    ...
    # Open file
    if (mode == 'w') & (os.path.isfile(fname)):
        os.remove(fname)
        ncid = nc.Dataset(fname, mode, format='NETCDF4')
    else:
        print(f'File {fname} already exist. Please use mode=''w'' or the name of the file.')
        return
